...
public class RocksDBSegmentStateBackendFactory implements StateBackendFactory<RocksDBStateBackend> {
	...
	public RocksDBStateBackend createFromConfig(
		Configuration config, ClassLoader classLoader) throws IllegalConfigurationException {
		...
		return new RocksDBStateBackend(new FsSegmentStateBackend(checkpointDirURI)).configure(config, classLoader);
	}
}
