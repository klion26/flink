...
import org.apache.flink.runtime.checkpoint.savepoint.SavepointSerializerUtil;
...
public class OperatorSnapshotUtil {
	...
	public static void writeStateHandle(OperatorSubtaskState state, String path) throws IOException {
		...
		try (DataOutputStream dos = new DataOutputStream(out)) {
			...
			SavepointSerializerUtil.serializeStreamStateHandle(null, dos);
			...
			if (rawOperatorState != null) {
				...
				for (OperatorStateHandle operatorStateHandle : rawOperatorState) {
					SavepointSerializerUtil.serializeOperatorStateHandle(operatorStateHandle, dos);
				}
			} else {
			...
			if (rawKeyedState != null) {
				...
				for (KeyedStateHandle keyedStateHandle : rawKeyedState) {
					SavepointSerializerUtil.serializeKeyedStateHandle(keyedStateHandle, dos);
				}
			} else {
	...
	public static OperatorSubtaskState readStateHandle(String path) throws IOException, ClassNotFoundException {
		...
		try (DataInputStream dis = new DataInputStream(in)) {
			...
			SavepointSerializerUtil.deserializeStreamStateHandle(dis);
			...
			if (numRawOperatorStates >= 0) {
				...
				for (int i = 0; i < numRawOperatorStates; i++) {
					OperatorStateHandle operatorState = SavepointSerializerUtil.deserializeOperatorStateHandle(
						dis);
			...
			if (numRawKeyedStates >= 0) {
				...
				for (int i = 0; i < numRawKeyedStates; i++) {
					KeyedStateHandle keyedState = SavepointSerializerUtil.deserializeKeyedStateHandle(
						dis);
